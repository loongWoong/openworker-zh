...
export function FolderGate({ onChoose, onCancel, create }: Props) {
  ...
  const open = async (p: string, doCreate = false) => {
    ...
    else setError(res.error || t.session.couldNotOpen);
  };
  ...
  return (
    <div className="gate-overlay">
      <div className="gate">
        ...
        <h2>{create ? t.session.newProject : t.session.chooseProjectFolder}</h2>
        <p className="gate-sub">
          {create
            ? t.session.newProjectSub
            : t.session.chooseProjectFolderSub}
        </p>
        ...
        <div className="gate-input">
          <input
            ...
            autoFocus
          />
          <button className="btn" onClick={browse} title={t.common.browse}>
            {t.common.browse}
          </button>
          <button className="btn primary" onClick={() => open(path, create)} disabled={!path.trim()}>
            {create ? t.common.create : t.common.open}
          </button>
        </div>
        ...
        {recents.length > 0 && (
          <>
            <div className="gate-label">{t.session.recentFolders}</div>
